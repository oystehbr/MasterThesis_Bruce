...
class AgentNode:
    ...
    def __init__(self, node, nodes, nodes_loading_or_dumping, exploration_num=0):
        ...
        self.best_memory = [0]*len(self.nodes)
        self.random_choice_prob = 5  # %
        self.times = 1
        self.exploration_break = 5
        self.restart_round_memory()

        # input, the number of
        self.input_size = len(self.nodes)  # 7
    ...
    def initialize_network(self):
        ...
        network = [self.input_size, self.output_size]
    ...
    def reset_memory(self):
        ...
        # self.best_memory_control = np.zeros(len(self.nodes))
    ...
    def train_long_memory(self):
        ...
        if len(self.memory) > BATCH_SIZE:
            mini_samples.append(random.sample(
                self.memory, BATCH_SIZE))  # list of tuples
        else:
            mini_samples.append(self.memory)

        _best_memory = [item for item in self.best_memory if item != 0]

        if len(_best_memory) > BATCH_SIZE:
            mini_samples.append(random.sample(
                _best_memory, BATCH_SIZE))  # list of tuples
        else:
            mini_samples.append(_best_memory)

        # If there is something to train
        for mini_sample in mini_samples:
    ...
    def get_action(self, state, dumper, choice='optimal'):

        if choice == 'optimal':
            return self.get_action_optimal(state, dumper)
        elif choice == 'policy':
            return self.get_action_policy(state, dumper)

    def get_action_policy(self, state, dumper):
    ...
    def get_action_optimal(self, state, dumper):
        # TODO: could save this information?

        to_node = self.nodes[np.where(state == 1)[0][0]]
        best_edge = self.node.get_best_edge_to_node(to_node)
        return best_edge
    ...
    def save_model(self, path):
        ...
        torch.save(self.model.state_dict(), f'{path}/{self.node.get_id()}')
            ...
            while not finished:
                ...
                try:
                    self.model.load_state_dict(
                        torch.load(f'{map_path}/{foldername}/{self.node.get_id()}'))
                    finished = True
    ...
    def add_route_to_round_memory(self, sample, train=True):
        if train:
